...
테이브의 모든행중 해당 컬럼에 값은 중복되지 않고 유일해야됨
...
FOREIGN KEY 제약조건
입력하고자 한느 데이터가 참조하는 텡블에 존재할 때만 입력 허용
ex : emp 테이블에 데이터를 입력할 때 deptno 커럼의 값이 dept 테이블에 존재하는 deptno값이어야 입력 가능
...
99번 부서는 존재 하므로 정삭적으로 입력 가능
INSERT INTO emp_test VALUES (9999, 'brown', 99);

98번 부서는 존재 하지 않으므로 정삭적으로 입력 할 수 없다.
...
존재하기 때문에 자식 레콛 입장에서는 데이터 무결성이 깨지게 되어
정상적으로 삭제 할 수 없다.
DELETE dept_test
WHERE deptno = 99;
...
2. 테이블 명세서가 정확하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
    (java코드 + 테이블 내역 확인 필요)
    java코드에는 dept테이블만 삭제한는 코드가 있는데
...
emp 테이블에서 급여정보(sal)를 관리 할때 sall 컬럼의 값은 0보다 작은 값이 들어가는 것은
로직적으로 이상함.
...
dept_tset 테이블을 복사 ==> dept_test_copy
CREATE TABLE dept_test_copy AS
SELECT *
FROM dept_test;

테이블 없이 테이블을 복사 하고 싶을 때, 컬럼만 복사하고 싶을 때
...
이미 생성된 테이블을 수정 할 수도 있음
     ...
     *******기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가할 수가 없다.
            ==> 기존 테이블을 삭제하고 컬럼순서를 조정한 새로운 테이블 생성 DDL로
                새롭게 테이블을 만들어야 함.
                운영중이라면 이미 데이터가 쌓여있을 가승성이 굉장히 높음
...
데이터가 존재하는 컬럼에 대해서는 타입 변경이 불가능 하다. --사이즈 증가를 가능
INSERT INTO emp_test VALUES (9999, 'brown', 99, sysdate);
ALTER TABLE emp_test MODIFY (hp VARCHAR2(30));


컬럼 이름 변경 -- 포린키 사용중이여도 괜찮
ALTER TABLE 테이블명 RENAME COLUMN 기존컬럼명 TO 신규컬럼명;

위에서 추가한 emp_tset 테이블의 hp 컬럼을 hp_n으로 이름 변경
