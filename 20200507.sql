...
ranger_no컬럼은 NUMBER 타입으로
ranger_nm컬럼은 VARCHAR2(50) 타입으로
reg_dt 컬럼은 DATE타입으로 (단 기본값은 입력당시 서버의 현재 시간을 저장)
...
1. 테이블 생성시 컬럼 옆에다가 해당 컬럼에 적용될 제약조건을 기술
   ==> 복합컬럼 설정이 불가
2. 테이블 생성 시 컬럼 기술과 별도로 해당 테이블에 적용될 제약조건을 기술
   ==> 복합컬럼 설정 가능
...
dept_tset 테이블에 deptno, dname으로 PRIMARY KEY 제약조건을 생성
두개 컬럼이 모두 같아야지만 중복되는행으로 인식
