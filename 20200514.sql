...
PC07.v_emp ==> v_emp 시노님으로 생성
CREATE SYNONYM v_emp FOR PC07.v_emp;
...
Data Dicrionay : 시스템 정보를 볼 수 있는 view, 오라클이 자체적으로 관리
category (접두어)
USER : 해당 사용자가 소유하고 있는 객체 목록
ALL : 해당 사용자 소유 + 권한 부여받은 객체 목록
DBA : 모든 객체 목록
vs : 성능, 시스템 관련
...
emp_tset 테이블을 이용하여 emp_test2 테이블 생성
...
위의경우는 테이블 ==> 다른 테이블로 머지
데이터를 ==> 특정 테이블로 머지

이번엔 하는 시나리오 : 데이터를 ==> 특정 테이블로 머지
(9999, james)
